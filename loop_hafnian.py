...
def _calc_loop_hafnian(A, D, edge_reps,
    ...
    H = 0  # start running total for the hafnian
    ...
    for j in range(rank, steps, size):
        ...
        if oddloop is not None:
            H += prefac * f_loop_odd(E, AX_S, XD_S, D_S, N, oddloop, oddVX_S)[N]
        else:
            if glynn and kept_edges[0] == 0:
                prefac *= 0.5
            H += prefac * f_loop(E, AX_S, XD_S, D_S, N)[N // 2]
...
def loop_hafnian(A, D=None, reps=None,
    ...
    H = _calc_loop_hafnian(Ax, Dx, edge_reps, oddloop, oddV, rank, size, glynn)
    return H
